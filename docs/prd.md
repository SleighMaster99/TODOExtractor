...
| 날짜 | 버전 | 설명 | 작성자 |
|------|------|------|--------|
| 2024-01-01 | 1.0 | 최초 PRD 작성 | 비즈니스 분석가 |
...
유닛 테스트만 - 핵심 파서 및 추출 로직에 대한 유닛 테스트
...
**목표**: C++17 표준 라이브러리만 사용하여 코드베이스를 스캔하고 마커를 추출하는 CLI 도구를 개발합니다. 사용자는 명령줄에서 실행하여 Markdown 리포트를 생성할 수 있습니다.

#### 스토리 1.1: 프로젝트 설정 및 기본 구조

**사용자로서** 개발자는,
**원하는 것은** 프로젝트 기본 구조와 빌드 시스템을 설정하는 것입니다,
**그 이유는** 이후 기능 개발을 위한 기반을 마련할 수 있기 때문입니다.
...
**사용자로서** 사용자는,
**원하는 것은** JSON 설정 파일에서 마커, 확장자, 제외 디렉터리를 읽는 것입니다,
**그 이유는** 추출 동작을 커스터마이즈할 수 있기 때문입니다.
...
#### 스토리 1.3: 디렉터리 재귀 스캐너 구현

**사용자로서** 사용자는,
**원하는 것은** 지정된 디렉터리를 재귀적으로 스캔하는 것입니다,
**그 이유는** 모든 소스 파일을 탐색할 수 있기 때문입니다.
...
**사용자로서** 사용자는,
**원하는 것은** 파일에서 `//마커`로 시작하는 주석을 추출하는 것입니다,
**그 이유는** TODO 항목을 수집할 수 있기 때문입니다.
...
#### 스토리 1.5: Markdown 리포트 생성기

**사용자로서** 사용자는,
**원하는 것은** 추출 결과를 Markdown 파일로 출력하는 것입니다,
**그 이유는** 가독성 좋은 리포트를 얻을 수 있기 때문입니다.
...
#### 스토리 1.6: CLI 인터페이스 및 인자 파싱

**사용자로서** 사용자는,
**원하는 것은** 명령줄 인자로 옵션을 전달하는 것입니다,
**그 이유는** 설정 파일 없이도 빠르게 실행할 수 있기 때문입니다.
...
#### 스토리 1.7: CSV 및 JSON 출력 지원

**사용자로서** 사용자는,
**원하는 것은** 결과를 CSV와 JSON으로도 출력하는 것입니다,
**그 이유는** 다른 도구와 연동할 수 있기 때문입니다.
...
**목표**: CLI 기능을 Visual Studio 2022 확장으로 통합하고, 사용자 친화적인 UI를 통해 마커 관리 및 결과 확인 기능을 제공합니다.

#### 스토리 2.1: VS 확장 프로젝트 설정

**사용자로서** 개발자는,
**원하는 것은** Visual Studio 확장 프로젝트를 생성하는 것입니다,
**그 이유는** VS 환경에서 도구를 사용할 수 있기 때문입니다.
...
**사용자로서** 사용자는,
**원하는 것은** UI를 통해 마커를 추가/삭제하는 것입니다,
**그 이유는** 코드 수정 없이 설정을 변경할 수 있기 때문입니다.
...
#### 스토리 2.3: CLI 연동 및 결과 표시

**사용자로서** 사용자는,
**원하는 것은** VS에서 스캔을 실행하고 결과를 보는 것입니다,
**그 이유는** IDE를 벗어나지 않고 작업할 수 있기 때문입니다.
